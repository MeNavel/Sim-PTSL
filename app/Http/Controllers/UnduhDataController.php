...
class UnduhDataController extends Controller
{
    ...
    public function desa2024($desa)
    {
        $model_name = '\\App\\Models\\' . $desa;
        $model = new $model_name;
        $data = $model->select('*')->whereYear('tanggal_pendataan', 2024)->get();
        ...
        // Simpan file excel
        $writer = new Xlsx($spreadsheet);
        $writer->save('Nominatif Pendaftaran PTSL Desa ' . $desa . ' Tahun 2024.xlsx');

        return response()->download(public_path('Nominatif Pendaftaran PTSL Desa ' . $desa . ' Tahun 2024.xlsx'))->deleteFileAfterSend(true);
    }
