...
class MundurejoController extends Controller
{
    ...
    {
        $this->middleware('permission:mundurejo-index|mundurejo-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:mundurejo-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:mundurejo-edit', ['only' => ['update']]);
        $this->middleware('permission:mundurejo-delete', ['only' => ['destroy']]);
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) use ($user) {
                    ...
                    if ($user->can('mundurejo-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('mundurejo.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('mundurejo.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('mundurejo-delete')) {
        ...
        return view('Desa.Mundurejo.index');
    }

    public function create()
    {
        $data = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'MUNDUREJO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();

        return view('Desa.Mundurejo.create', compact('data'));
    }

    public function edit($id)
    {
        $data = Mundurejo::find($id);

        $koordinator = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'MUNDUREJO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();
        return view('Desa.Mundurejo.edit', compact('data', 'koordinator'));
    }

    /**
     * @throws Throwable
     */
    public function update(Request $request, Mundurejo $mundurejo)
    {
        DB::beginTransaction();
        try {
            $mundurejo->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('mundurejo.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    {
        if ($request->get('id')) {
            $data = Mundurejo::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = Mundurejo::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    public function berkas($No_Nominatif)
    {
        ...
        $phpWord->saveAs($data->id . ' - MUNDUREJO.docx');
        return response()->download(public_path($data->id . ' - MUNDUREJO.docx'))->deleteFileAfterSend();
    }
}
