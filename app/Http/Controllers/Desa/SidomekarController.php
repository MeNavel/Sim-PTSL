...
class SidomekarController extends Controller
{
    ...
    {
        $this->middleware('permission:sidomekar-index|sidomekar-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:sidomekar-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:sidomekar-edit', ['only' => ['update']]);
        $this->middleware('permission:sidomekar-delete', ['only' => ['destroy']]);
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) use ($user) {
                    ...
                    if ($user->can('sidomekar-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('sidomekar.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('sidomekar.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('sidomekar-delete')) {
        ...
        return view('Desa.Sidomekar.index');
    }

    public function create()
    {
        $data = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'SIDOMEKAR')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();

        return view('Desa.Sidomekar.create', compact('data'));
    }

    public function edit($id)
    {
        $data = Sidomekar::find($id);

        $koordinator = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'SIDOMEKAR')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();
        return view('Desa.Sidomekar.edit', compact('data', 'koordinator'));
    }

    /**
     * @throws Throwable
     */
    public function update(Request $request, Sidomekar $sidomekar)
    {
        DB::beginTransaction();
        try {
            $sidomekar->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('sidomekar.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    {
        if ($request->get('id')) {
            $data = Sidomekar::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = Sidomekar::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    public function berkas($No_Nominatif)
    {
        ...
        $phpWord->saveAs($data->id . ' - SIDOMEKAR.docx');
        return response()->download(public_path($data->id . ' - SIDOMEKAR.docx'))->deleteFileAfterSend();
    }
}
