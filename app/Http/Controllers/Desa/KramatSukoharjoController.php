...
class KramatSukoharjoController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:kramatsukoharjo-index|kramatsukoharjo-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:kramatsukoharjo-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:kramatsukoharjo-edit', ['only' => ['update']]);
        $this->middleware('permission:kramatsukoharjo-delete', ['only' => ['destroy']]);
    }
    ...
    {
        if ($request->ajax()) {
            $data = KramatSukoharjo::select('*');
            $user = auth()->user();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) use ($user) {
                    $btn = '<div class="btn-group ">';
                    if ($user->can('kramatsukoharjo-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('kramatsukoharjo.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('kramatsukoharjo.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('kramatsukoharjo-delete')) {
        ...
        return view('Desa.KramatSukoharjo.index');
    }
     ...
     * @throws Throwable
     */
    public function update(Request $request, KramatSukoharjo $kramatsukoharjo)
    {
        DB::beginTransaction();
        try {
            $kramatsukoharjo->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('kramatsukoharjo.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            KramatSukoharjo::create($request->all());
            DB::commit();
    ...
    {
        try {
            KramatSukoharjo::find($id)->delete();
    ...
    {
        if ($request->get('id')) {
            $data = KramatSukoharjo::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = KramatSukoharjo::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    {
        $data = KramatSukoharjo::find($No_Nominatif);
        ...
        $phpWord->saveAs($data->id . ' - KRAMAT SUKOHARJO.docx');
        return response()->download(public_path($data->id . ' - KRAMAT SUKOHARJO.docx'))->deleteFileAfterSend();
    }
