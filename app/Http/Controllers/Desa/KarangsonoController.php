...
class KarangsonoController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:karangsono-index|karangsono-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:karangsono-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:karangsono-edit', ['only' => ['update']]);
        $this->middleware('permission:karangsono-delete', ['only' => ['destroy']]);
    }
    ...
    {
        if ($request->ajax()) {
            $data = Karangsono::select('*');
            $user = auth()->user();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) use ($user) {
                    $btn = '<div class="btn-group ">';
                    if ($user->can('karangsono-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('karangsono.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('karangsono.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('karangsono-delete')) {
        ...
        return view('Desa.Karangsono.index');
    }

    public function create()
    {
        $data = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'KARANGSONO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();

        return view('Desa.Karangsono.create', compact('data'));
    }

    public function edit($id)
    {
        $data = Karangsono::find($id);

        $koordinator = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'KARANGSONO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();
        return view('Desa.Karangsono.edit', compact('data', 'koordinator'));
    }

    /**
     * @throws Throwable
     */
    public function update(Request $request, Karangsono $karangsono)
    {
        DB::beginTransaction();
        try {
            $karangsono->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('karangsono.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    {
        if ($request->get('id')) {
            $data = Karangsono::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = Karangsono::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    {
        $data = Karangsono::find($No_Nominatif);
        ...
        $phpWord = new TemplateProcessor('karangsono2024.docx');

        $kades = "MOHAMAD AHRUL FATAH";
        $awal_nib = "12.34.11.01.";
        ...
        $phpWord->saveAs($data->id . ' - KARANGSONO.docx');
        return response()->download(public_path($data->id . ' - KARANGSONO.docx'))->deleteFileAfterSend();
    }
