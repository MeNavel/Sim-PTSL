...
class PondokjoyoController extends Controller
{
    ...
    {
        $this->middleware('permission:pondokjoyo-index|pondokjoyo-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:pondokjoyo-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:pondokjoyo-edit', ['only' => ['update']]);
        $this->middleware('permission:pondokjoyo-delete', ['only' => ['destroy']]);
    }
    ...
    {
        if ($request->ajax()) {
            $data = Pondokjoyo::select('*');
            $user = auth()->user();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) use ($user) {
                    $btn = '<div class="btn-group ">';
                    if ($user->can('pondokjoyo-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('pondokjoyo.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('pondokjoyo.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('pondokjoyo-delete')) {
        ...
        return view('Desa.Pondokjoyo.index');
    }

    public function create()
    {
        $data = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'PONDOK JOYO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();

        return view('Desa.Pondokjoyo.create', compact('data'));
    }

    public function edit($id)
    {
        $data = Pondokjoyo::find($id);

        $koordinator = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'PONDOK JOYO')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();
        return view('Desa.Pondokjoyo.edit', compact('data', 'koordinator'));
    }

    /**
     * @throws Throwable
     */
    public function update(Request $request, Pondokjoyo $pondokjoyo)
    {
        DB::beginTransaction();
        try {
            $pondokjoyo->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('pondokjoyo.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    {
        if ($request->get('id')) {
            $data = Pondokjoyo::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = Pondokjoyo::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    public function berkas($No_Nominatif)
    {
        ...
        $phpWord = new TemplateProcessor('pondokjoyo2023.docx');
        ...
        $phpWord->saveAs($data->id . ' - PONDOKJOYO.docx');
        return response()->download(public_path($data->id . ' - PONDOKJOYO.docx'))->deleteFileAfterSend();
    }
}
