...
class PatemonController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:patemon-index|patemon-edit|', ['only' => ['index', 'edit']]);
        $this->middleware('permission:patemon-create', ['only' => ['create', 'store']]);
        $this->middleware('permission:patemon-edit', ['only' => ['update']]);
        $this->middleware('permission:patemon-delete', ['only' => ['destroy']]);
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) use ($user) {
                    ...
                    if ($user->can('patemon-index')) {
                        $btn = $btn . '<a class="btn btn-info btn-sm" href="' . route('patemon.edit', $row->id) . '" role="button"><i class="bi bi-info-circle"></i></a>';
                    }
                    $btn = $btn . '<a class="btn btn-primary btn-sm" href="' . route('patemon.berkas', $row->id) . '" role="button"><i class="bi bi-file-earmark-word"></i></a>';
                    if ($user->can('patemon-delete')) {
        ...
        return view('Desa.Patemon.index');
    }
    ...
    public function create()
    {
        ...
        return view('Desa.Patemon.create', compact('data'));
    }

    public function edit($id)
    {
        $data = Patemon::find($id);

        $koordinator = Koordinator::select('id', 'nama')
            ->where('desa', '=', 'PATEMON')
            ->where('jabatan', '=', 'SAKSI 1')
            ->get();
        return view('Desa.Patemon.edit', compact('data', 'koordinator'));
    }

    /**
     * @throws Throwable
     */
    public function update(Request $request, Patemon $patemon)
    {
        DB::beginTransaction();
        try {
            $patemon->update($request->all());
            DB::commit();
            $message = "Nominatif " . $request->id . " berhasil diupdate";
            return redirect()->route('patemon.index')->with('success', $message);
        } catch (Throwable $e) {
            DB::rollback();
    ...
    {
        if ($request->get('id')) {
            $data = Patemon::find($request->get('id'));
            if ($data) {
    ...
    public function cek_nib(Request $request)
    {
        if ($request->get('nib')) {
            $nib = $request->get('nib');
            $data = Patemon::select('id', 'nama')->where('nib', $nib)->get();
            if (count($data) > 0) {
    ...
    {
        $data = Patemon::find($No_Nominatif);
        ...
        $phpWord->saveAs($data->id . ' - PATEMON.docx');
        return response()->download(public_path($data->id . ' - PATEMON.docx'))->deleteFileAfterSend();
    }
