...
@section('content')
    <div class="pagetitle">
        <h1>Edit Koordinator Desa</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('koordinator.index') }}">Koordinator</a></li>
                <li class="breadcrumb-item active">Edit Koordinator NIK {{($data->id)}}</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    ...
    <div class="card col-12">
        <div class="card-body">
            ...
            <form action="{{ route('koordinator.update', $data->id) }}" id="form_pengisian" method="post">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" id="desa" name="desa" aria-label="State">
                            @foreach ($pengajuan_desa as $desa)
                                <option value="{{ $desa->desa }}" @if ($data->desa == $desa->desa)
                                    {{ 'selected' }}
                                    @endif>{{ $desa->desa }}
                                </option>
                            @endforeach
