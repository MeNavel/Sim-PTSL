...
@section('content')
    <div class="pagetitle">
        <h1>Formulir Pendaftaran Sertifikat Tanah Desa Kramat Sukoharjo</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('kramatsukoharjo.index') }}">Kramat Sukoharjo</a></li>
    ...
    <form action="{{ route('kramatsukoharjo.store') }}" method="POST">
        @csrf
        <div class="row">
            {{--Spesifikasi Data--}}
            <div class="col-lg-4">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" id="dusun" name="dusun" aria-label="State">
                                    <option value="SUKO TIMUR" selected>SUKO TIMUR</option>
                                    <option value="SUKO BARAT">SUKO BARAT</option>
                                </select>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="status_penggunaan" class="col-sm-3 col-form-label">Keadaan</label>
                            <div class="col-sm-9">
                                <select class="form-select" id="status_penggunaan" name="status_penggunaan"
                                        aria-label="State">
                                    <option value="TANAH PEKARANGAN" selected>TANAH PEKARANGAN</option>
                                    <option value="TANAH PEKARANGAN BERDIRI BANGUNAN">TANAH PEKARANGAN BERDIRI BANGUNAN</option>
        ...
        $(document).ready(function () {
            ...
            $('#id').blur(function () {
                ...
                if (id === "") {
                    ...
                    $.ajax({
                        url: "{{ route('kramatsukoharjo.cek_nominatif') }}",
                        method: "POST",
                        data: {
                            id: id,
                            _token: _token
                        ...
                        success: function (result) {
                            if (result.success === false) {
                                ...
                                document.getElementById("hasil_cek").addEventListener("click", function() {
                                    ...
                                    // Membentuk URL dengan parameter
                                    var url = "{{ route('kramatsukoharjo.edit', ':kramatsukoharjo') }}";
                                    url = url.replace(':kramatsukoharjo', inputValue);

                                    // Mengarahkan halaman ke URL yang dibentuk
            ...
            $('#nib').blur(function () {
                ...
                if (nib === "") {
                    ...
                    $.ajax({
                        url: "{{ route('kramatsukoharjo.cek_nib') }}",
                        method: "POST",
                        data: {
                            nib: nib,
                            _token: _token
